...
\subsubsection{Pembentukan Voxel}

Ketika kedalaman rekursi mencapai \texttt{kedalamanMaks}, proses subdivisi dihentikan dan
node diklasifikasikan sebagai \textit{leaf node}. Pada tahap ini, setiap node merepresentasikan
sebuah kubus kecil dalam ruang 3D. Jika node tersebut tidak dipangkas (artinya masih beririsan dengan permukaan model), maka
node dianggap sebagai bagian dari representasi objek dan ditulis sebagai satu voxel berbentuk
kubus ke berkas OBJ. Setiap voxel direpresentasikan menggunakan 8 titik sudut dan 12 bidang segitiga. Dengan cara ini,
permukaan objek yang semula kontinu didekati oleh kumpulan kubus-kubus kecil hasil subdivisi.
